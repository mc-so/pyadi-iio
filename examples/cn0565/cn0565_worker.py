...
class CN0565_Worker(QThread):
    ...
    def run(self):
        self.exiting=False
        # self.supported_electrode_count = self.intf.supported_electrode_count()
        # self.doneGetSupportedElectrodeCount.emit(self.supported_electrode_count)

        print("Electrode count is:", self.el)
        self.intf.mode(freq=self.freq,
                                el=self.el,
        ...
        protocol_obj = protocol.create(self.el, self.dist_exc, self.step_meas, self.parser_meas)
        ...
        while not self.exiting:
            voltages = self.intf.read_boundary_voltages()
    ...
    def solver(self, v0, v1, protocol_obj):
        if self.reconstruction=="bp":
            ...
            self.eit.setup(p=0.5, lamb=0.01, method="kotre", perm=1, jac_normalized=True)
            # the normalize for BP when dist_exc>4 should always be True
            self.ds = self.eit.solve(v1, v0, normalize=True)
            self.ds = sim2pts(self.mesh_obj.node, self.mesh_obj.element, self.ds) #changes here
            self.plot(self.figure, self.mesh_obj, self.ds)
            self.doneCompute.emit()
